...
#ifdef CHINA_POST
#define CLIENT_CERT_NAME "mdolphin_client_cert"
#endif
...
int saveCertificateHome(X509 * cert, bool isClient)
{
    ...
    if (dir) {
        ...
        do{
            if (isClient){
#ifdef CHINA_POST
                snprintf(buf,4096,"%s/.mDolphin/certs/%s.%s",
                        dir, CLIENT_CERT_NAME, "crt");
#else
...
static int loadPem(const char *fn)
{
    X509 *x509;
    int ret;
    FILE *fp = fopen(fn, "r");
	...
	if (x509)
	{
		ret = saveCertificateHome(x509, false);
		X509_free(x509);
	}
	else
	{
...
static BOOL mdolphin_add_server_certificate(const char* fn)
{
    if ( !loadPem(fn))
        return true;
    else
...
BOOL importServerCertificate(HWND hParent)
{
    ...
    if(ret == IDOK){
        ...
        if (mdolphin_add_server_certificate(fullname)) {
            MessageBox( hParent, "Successfully restored your security certificate(s)",
                    "Alert", MB_OK);
            result = true;
        } else {
            MessageBox( hParent, "Error parsing certificate (not Base64?)",
                    "Alert", MB_OK);
            result = false;
        }
        if (fullname)
...
static int loadPkcs12( const char *fn, const char* pw)
{
    ...
    ret = PKCS12_parse(p12, pw, &pkey, &x509, NULL);
    if (ret != 1) {
		PKCS12_free(p12);
...
BOOL importCertificate(HWND hParent)
{
    ...
    NEWFILEDLGDATA newfiledlg =
    {
        ...
        "",
        "./",
        "All file(*.*)|Pkcs12(*.pfx)|Pkcs12(*.p12)",
        1
    };
    int ret = ShowOpenDialog (hParent, 100, 100, 350, 250, &newfiledlg);
    if(ret == IDOK){
...
BOOL readCertificateHome(unsigned long hash, X509 **x509, EVP_PKEY **pkey)
{
    ...
    if(dir) {
#ifdef CHINA_POST
        snprintf(buf1, 4095, "%s/.mDolphin/certs/%s.%s", dir, CLIENT_CERT_NAME, "crt");
        snprintf(buf2, 4095, "%s/.mDolphin/certs/%s.%s", dir, CLIENT_CERT_NAME, "key");
#else
