...
#define IDC_SET_ENABLE_DEVELOPER_EXTRAS 625 /*FALSE*/
...
static void std_font_notif_proc(HWND hDlg, long int id, int nc, DWORD add_data)
{
    int num;
    if (nc == CBN_SELCHANGE) {
        num = SendMessage(hDlg, CB_GETCURSEL, 0, 0);
...
//static void show_all_at_once_notif_proc(HWND hwnd, int id, int nc, DWORD add_data)		// gengyue
static void show_all_at_once_notif_proc(HWND hwnd, long int id, int nc, DWORD add_data)
{
    if (nc == BN_CLICKED) {
        if (SendMessage(hwnd, BM_GETCHECK, 0, 0) == BST_CHECKED )
...
//static void developer_extras_notif_proc(HWND hwnd, int id, int nc, DWORD add_data)		// gengyue
static void developer_extras_notif_proc(HWND hwnd, long int id, int nc, DWORD add_data)
{
    if (nc == BN_CLICKED) {
        if (SendMessage(hwnd, BM_GETCHECK, 0, 0) == BST_CHECKED )
            setup_info.enable_developer_extras = TRUE;
        else
            setup_info.enable_developer_extras = FALSE;
    }
}
...
static void set_notify_proc(HWND hDlg)
{
    ...
    SetNotificationCallback (GetDlgItem (hDlg, IDC_SET_ENABLE_DEVELOPER_EXTRAS), developer_extras_notif_proc);
...
{ 
    ...
    if (setup_info.enable_developer_extras)
        SendMessage (GetDlgItem (hDlg, IDC_SET_ENABLE_DEVELOPER_EXTRAS), BM_SETCHECK, BST_CHECKED, 0);
    else
        SendMessage (GetDlgItem (hDlg, IDC_SET_ENABLE_DEVELOPER_EXTRAS), BM_SETCHECK, BST_UNCHECKED, 0);
...
static CTRLDATA CtrlInitProgress [] =
{
    ...
    
    {
        ...
        WS_TABSTOP | WS_VISIBLE | BS_AUTOCHECKBOX | BS_NOTIFY,
        10, 350, 220, 20,
        IDC_SET_ENABLE_DEVELOPER_EXTRAS,
...
static LRESULT SettingsProc (HWND hDlg, unsigned int message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) {
        case MSG_INITDIALOG: 
            ...
            MoveWindow (hDlg , (rc.right-rc.left)/2-168 , (rc.bottom-rc.top)/2-195, 436 , 450, TRUE);
            return (LRESULT)1;
        case MSG_COMMAND:
            switch (wParam) {
#if ENABLE_PLUGIN   
